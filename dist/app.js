...
function ratingFor(paper) {
  const ratings = JSON.parse(localStorage.getItem('arxiv-shelf-ratings') || '{}');
  return Number(Object.hasOwn(ratings, paper.catalog_key) ? ratings[paper.catalog_key] : paper.rating || 0);
}
...
function render() {
  ...
  selected.sort((a, b) => {
    let av = sort === 'authors' ? (a.authors || [])[0] || '' : sort === 'rating' ? ratingFor(a) : a[sort] || '';
    let bv = sort === 'authors' ? (b.authors || [])[0] || '' : sort === 'rating' ? ratingFor(b) : b[sort] || '';
    if (!av && bv) return 1;
    if (av && !bv) return -1;
